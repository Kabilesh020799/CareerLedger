...
Google OAuth credentials are encrypted before storage. A manual synchronization request validates the connection and enqueues a user-scoped BullMQ job, then returns without waiting for Gmail or model-provider latency. The frontend polls short, authenticated status requests until the worker completes or reports a sanitized failure. The worker fetches message metadata incrementally using Gmail history identifiers and deduplicates by Gmail message ID. Each stored reference records the classifier version last applied, allowing a newer rule set to re-evaluate older messages once without repeatedly scanning them. Deterministic rules remain the primary classifier. Only messages for which those rules return no result and whose signed-in application account is on the server-side OpenAI allowlist are eligible for the optional fallback. Eligible messages use bounded concurrency so one slow provider call does not serialize the entire batch. The backend disables response storage (`store: false`), requests structured output, validates its schema and confidence, and converts an accepted result into a pending review. Missing configuration, unauthorized accounts, timeouts, provider errors, non-success responses, malformed output, and low confidence all resolve to no classification without failing synchronization. Suggested changes remain pending until the user confirms, ignores, or creates an application.
