...
`GET /api/admin/users` requires both an authenticated session and an application login email configured in `ADMIN_ACCOUNT_EMAILS`. It accepts `page`, `pageSize`, and optional `search` query parameters and returns aggregate account totals plus paginated account metadata. It never returns password hashes, Google identifiers, sessions, tokens, or private application content. The authentication session response includes a server-derived `isAdmin` flag so the frontend can hide administrator navigation; the API authorization check remains authoritative.
