...
Attach an EC2 instance role that can perform `s3:PutObject`, `s3:GetObject`, and `s3:DeleteObject` on the private document bucket's `resumes/*` prefix. Cover letters use `resumes/cover-letters/*`, so existing least-privilege resume roles remain compatible. Keep all S3 Block Public Access controls enabled. Because the backend runs in Docker and uses IMDSv2 credentials, enable the metadata endpoint, require IMDSv2, and set the metadata response hop limit to `2`. No static AWS access keys are needed.
...
Restrict the environment to the protected `master` branch. Its required pull-request status checks are `Verify release metadata`, `Audit npm dependencies`, `Verify backend`, `Verify frontend`, `Verify critical browser workflows (1/2)`, `Verify critical browser workflows (2/2)`, and `Verify deployment automation`. Dependabot can queue only npm patch updates for squash merge after those checks pass; major and minor dependency updates require manual review. Add required approval if deployments should pause for confirmation after images are published.
...
The IAM trust policy must use the exact subject emitted by this repository's GitHub OIDC customization: `repo:OWNER@OWNER_ID/REPOSITORY@REPOSITORY_ID:environment:production`. For this repository, the value is `repo:Kabilesh020799@47252881/CareerLedger@1326925254:environment:production`. If the repository owner, name, or OIDC subject customization changes, update `github_oidc_subject` in `infrastructure/production/terraform.tfvars` and apply a reviewed Terraform plan before the next release.

Configure the document bucket CORS policy to allow `GET`, `HEAD`, and `POST` only from the production CloudFront origin. Add lifecycle rules that expire objects under `resumes/pending/` and `resumes/cover-letters/pending/` after one day. The application promotes verified uploads to each document type's `active/` prefix, so active attachments are not affected. On startup, the backend idempotently moves legacy PostgreSQL-backed resume bytes into S3 and retains the database copy whenever a migration upload cannot be completed.
