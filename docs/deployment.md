...
The IAM trust policy must use GitHub's exact environment subject format: `repo:OWNER/REPOSITORY:environment:production`. For this repository, the value is `repo:Kabilesh020799/CareerLedger:environment:production`. If the repository name changes, update `github_oidc_subject` in `infrastructure/production/terraform.tfvars` and apply a reviewed Terraform plan before the next release.
