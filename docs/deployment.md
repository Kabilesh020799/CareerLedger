...
The Prometheus, Grafana, cAdvisor, and exporter containers are disabled by default. Set `OBSERVABILITY_ENABLED=true` in the protected production `.env` before deployment to start them; leave it `false` on hosts with less than 4 GiB RAM. Changing it back to `false` and deploying stops any running observability containers while leaving the application, PostgreSQL, Redis, and their named volumes intact.

When observability is enabled, Prometheus retains at most 15 days or 2 GB by default; adjust `PROMETHEUS_RETENTION` and `PROMETHEUS_RETENTION_SIZE` only after checking disk headroom. Grafana is available only on EC2 loopback port `3001`. Start an SSM port-forwarding session to `3001` (or an SSH tunnel such as `ssh -L 3001:127.0.0.1:3001 ...`) and open `http://127.0.0.1:3001`. Read `GRAFANA_ADMIN_USER` and `GRAFANA_ADMIN_PASSWORD` directly from the protected instance `.env`; do not place them in GitHub logs or chat. The provisioned dashboards cover request RED metrics, queues and Gmail failures, PostgreSQL, Redis, Nginx, host, and Docker saturation.
