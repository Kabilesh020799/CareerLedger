...
export const openApiDocument: OpenAPIV3.Document = {
    ...
    schemas: {
      Application: {
        ...
        properties: {
          ...
          resumeAttachment: { type: "object", nullable: true, properties: { fileName: { type: "string" }, mimeType: { type: "string" }, size: { type: "integer" }, createdAt: { type: "string", format: "date-time" } } },
          coverLetterAttachment: { type: "object", nullable: true, properties: { fileName: { type: "string" }, mimeType: { type: "string" }, size: { type: "integer" }, createdAt: { type: "string", format: "date-time" } } },
          sprint: { allOf: [{ $ref: "#/components/schemas/Sprint" }], nullable: true },
      ...
      ApplicationInput: {
        ...
        properties: {
          ...
          location: { type: "string", nullable: true }, jobUrl: { type: "string", format: "uri", pattern: "^https?://", nullable: true, description: "Absolute HTTP or HTTPS job URL." },
          source: { type: "string", nullable: true }, notes: { type: "string", nullable: true },
          status: { $ref: "#/components/schemas/ApplicationStatus" }, appliedAt: { type: "string", format: "date-time", nullable: true },
          resumeVersionId: { type: "string", nullable: true }, resumeUploadKey: { type: "string" }, coverLetterUploadKey: { type: "string" },
        },
      },
      ApplicationStatus: { type: "string", enum: ["SAVED", "APPLIED", "SCREENING", "ASSESSMENT", "INTERVIEW", "OFFER", "REJECTED", "WITHDRAWN"] },
      Sprint: {
        type: "object",
        required: ["id", "name", "sequence", "status", "durationDays", "startedAt", "scheduledStartAt", "endsAt", "createdAt", "updatedAt"],
        properties: {
          id: { type: "string" },
          userId: { type: "string" },
          ...
          status: { type: "string", enum: ["ACTIVE", "CLOSED", "SCHEDULED"] },
      ...
      SprintScheduleConflict: {
        ...
        properties: {
          error: { type: "string" },
          scheduledStartAt: { type: "string", format: "date-time", nullable: true },
          requiredStartAt: { type: "string", format: "date-time", nullable: true },
        },
      },
      ...
      GmailSyncResult: { type: "object", required: ["synchronizationType", "fetchedMessages", "newMessages", "duplicateMessages", "analyzedMessages", "detectedUpdates", "lastSyncedAt"], properties: { synchronizationType: { type: "string", enum: ["full", "incremental"] }, fetchedMessages: { type: "integer" }, newMessages: { type: "integer" }, duplicateMessages: { type: "integer" }, analyzedMessages: { type: "integer" }, detectedUpdates: { type: "integer" }, lastSyncedAt: { type: "string", format: "date-time" } } },
      GmailSyncStart: { type: "object", required: ["jobId", "status"], properties: { jobId: { type: "string" }, status: { type: "string", enum: ["queued", "running"] } } },
      GmailSyncJobStatus: { type: "object", required: ["jobId", "status"], properties: { jobId: { type: "string" }, status: { type: "string", enum: ["queued", "running", "completed", "failed"] }, result: { $ref: "#/components/schemas/GmailSyncResult" }, error: { type: "string" } } },
  ...
  paths: {
    ...
    "/api/applications": {
      ...
      post: { tags: ["Applications"], summary: "Create an application", description: "Adds a company and role to the user's job-search pipeline. A resume, cover letter, or both can be attached before saving.", security: [{ sessionCookie: [] }], requestBody: { required: true, content: { "application/json": { schema: { $ref: "#/components/schemas/ApplicationInput" } } } }, responses: { "201": { description: "Application created." }, "400": { description: "Validation error" } } },
    },
    ...
    "/api/sprints/schedule": { post: { tags: ["Sprints"], summary: "Schedule an upcoming sprint", description: "Creates a future scheduled sprint after the active sprint and all existing scheduled plans. The plan stores its scheduled start and planned end without changing application assignments.", security: [{ sessionCookie: [] }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["startsAt"], properties: { name: { type: "string", minLength: 1, maxLength: 100 }, durationDays: { type: "integer", minimum: 1, maximum: 90 }, startsAt: { type: "string", format: "date-time" } } } } } }, responses: { "201": { description: "Sprint scheduled.", content: { "application/json": { schema: { $ref: "#/components/schemas/Sprint" } } } }, "400": { description: "Invalid scheduled sprint data." }, "401": { description: "Authentication required." }, "403": { description: "Selected workspace is read-only." }, "404": { description: "Selected workspace was not found." }, "409": { description: "The requested start time is in the past or overlaps an existing sprint plan.", content: { "application/json": { schema: { $ref: "#/components/schemas/SprintScheduleConflict" } } } } } } },
    "/api/sprints/start": { post: { tags: ["Sprints"], summary: "Start the next sprint", description: "After the active sprint reaches its configured end time, closes it and creates the next sprint in one transaction. Rejected applications remain in the closed sprint; all other applications carry over. With scheduledSprintId, activates only the next scheduled plan after its start time has arrived. Starting early or selecting a plan out of order returns 409 without mutating sprint or application data.", security: [{ sessionCookie: [] }], requestBody: { required: false, content: { "application/json": { schema: { type: "object", properties: { name: { type: "string", minLength: 1, maxLength: 100 }, durationDays: { type: "integer", minimum: 1, maximum: 90, description: "Whole-day sprint duration. Defaults to 14 for the first sprint or inherits the active sprint duration thereafter." }, scheduledSprintId: { type: "string", description: "Activates the next eligible scheduled sprint when its planned start has arrived." } } } } } }, responses: { "201": { description: "Sprint started.", content: { "application/json": { schema: { $ref: "#/components/schemas/SprintStartResult" } } } }, "400": { description: "Invalid sprint name, duration, or scheduled sprint ID." }, "401": { description: "Authentication required." }, "403": { description: "Selected workspace is read-only." }, "404": { description: "Selected workspace was not found." }, "409": { description: "The active sprint has not ended, or the scheduled plan is not ready or next in order.", content: { "application/json": { schema: { oneOf: [{ $ref: "#/components/schemas/SprintActiveConflict" }, { $ref: "#/components/schemas/SprintScheduleConflict" }] } } } } } } },
    "/api/sprints/{id}": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }],
      patch: { tags: ["Sprints"], summary: "Edit an upcoming sprint", description: "Updates the name, duration, and/or scheduled start of a future sprint plan. The plan must remain after the active sprint and before the next scheduled plan; application assignments are unchanged.", security: [{ sessionCookie: [] }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", minProperties: 1, properties: { name: { type: "string", minLength: 1, maxLength: 100 }, durationDays: { type: "integer", minimum: 1, maximum: 90 }, startsAt: { type: "string", format: "date-time" } } } } } }, responses: { "200": { description: "Updated sprint plan.", content: { "application/json": { schema: { $ref: "#/components/schemas/Sprint" } } } }, "400": { description: "Invalid scheduled sprint data." }, "401": { description: "Authentication required." }, "403": { description: "Selected workspace is read-only." }, "404": { description: "Scheduled sprint not found." }, "409": { description: "The requested plan is in the past or overlaps another sprint plan.", content: { "application/json": { schema: { $ref: "#/components/schemas/SprintScheduleConflict" } } } } } },
      delete: { tags: ["Sprints"], summary: "Cancel an upcoming sprint", description: "Removes a future sprint plan without changing application assignments.", security: [{ sessionCookie: [] }], responses: { "204": { description: "Sprint plan canceled." }, "401": { description: "Authentication required." }, "403": { description: "Selected workspace is read-only." }, "404": { description: "Scheduled sprint not found." } } },
    },
    ...
    "/api/applications/cover-letter-uploads": {
      post: { tags: ["Cover Letters"], summary: "Prepare a cover-letter upload", description: "Validates PDF, DOC, or DOCX metadata up to 5 MB and returns private S3 form fields or database fallback instructions.", security: [{ sessionCookie: [] }], responses: { "200": { description: "Database fallback mode." }, "201": { description: "Private S3 upload preparation." }, "400": { description: "Unsupported or oversized file." } } },
      delete: { tags: ["Cover Letters"], summary: "Abandon a pending cover-letter upload", description: "Deletes a pending object owned by the current user.", security: [{ sessionCookie: [] }], responses: { "204": { description: "Pending upload removed." }, "404": { description: "Owned upload not found." } } },
    },
    ...
    "/api/applications/{id}/cover-letter-download": { get: { tags: ["Cover Letters"], summary: "Prepare a private cover-letter download", security: [{ sessionCookie: [] }], parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Download preparation." }, "404": { description: "Application or cover letter not found." } } } },
    "/api/applications/{id}/cover-letter": { get: { tags: ["Cover Letters"], summary: "View an attached cover letter", description: "Returns an owned cover letter inline or redirects to a short-lived private S3 URL.", security: [{ sessionCookie: [] }], parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Inline cover-letter bytes." }, "302": { description: "Redirect to a short-lived inline S3 URL." }, "404": { description: "Cover letter not found." } } } },
    "/api/applications/{id}": {
      parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }],
      get: { tags: ["Applications"], summary: "Get an application", description: "Loads complete application details, including attached resume and cover-letter metadata.", security: [{ sessionCookie: [] }], responses: { "200": { description: "Application", content: { "application/json": { schema: { $ref: "#/components/schemas/Application" } } } }, "404": { description: "Not found" } } },
      patch: { tags: ["Applications"], summary: "Update an application", description: "Changes editable fields or replaces an attached resume or cover letter. Status changes also create a timeline event transactionally.", security: [{ sessionCookie: [] }], responses: { "200": { description: "Updated" }, "400": { description: "Validation error" } } },
    ...
    "/api/gmail/sync": { post: { tags: ["Gmail"], summary: "Queue a Gmail synchronization", description: "Validates the signed-in user's Gmail connection, queues incremental synchronization for the background worker, and returns without waiting for Gmail or optional model-provider calls.", security: [{ sessionCookie: [] }], responses: { "202": { description: "Synchronization queued or already running.", content: { "application/json": { schema: { $ref: "#/components/schemas/GmailSyncStart" } } } }, "409": { description: "Gmail is not connected." }, "503": { description: "Gmail synchronization queue is temporarily unavailable." } } } },
    "/api/gmail/sync/{id}": { get: { tags: ["Gmail"], summary: "Get Gmail synchronization progress", description: "Returns the current state of a manual synchronization owned by the signed-in user. Provider failures are represented by a fixed safe message.", security: [{ sessionCookie: [] }], parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Current synchronization status.", content: { "application/json": { schema: { $ref: "#/components/schemas/GmailSyncJobStatus" } } } }, "404": { description: "Synchronization was not found for this user." }, "503": { description: "Gmail synchronization queue is temporarily unavailable." } } } },
