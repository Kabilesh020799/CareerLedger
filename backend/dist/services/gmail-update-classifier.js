...
exports.classifyGmailMessage = classifyGmailMessage;
exports.matchGmailMessage = matchGmailMessage;
exports.inferCompany = inferCompany;
exports.inferJobTitle = inferJobTitle;
...
function matchGmailMessage(message, applications) {
    ...
    const best = scored[0];
    const next = scored[1];
    if (!best || best.score < 55 || next?.score === best.score)
        return null;
...
function inferCompany(sender) {
    ...
    if (!domainPart || /^(gmail|outlook|hotmail|yahoo)$/i.test(domainPart))
        return "";
    return titleCase(domainPart.replace(/[-_]+/g, " "));
}
function inferJobTitle(subject) {
...
function applicationMatchScore(message, application) {
    ...
    let score = 0;
    if (company.length >= 3 && subject.includes(company))
        score += 45;
    if (jobTitle.length >= 4 && subject.includes(jobTitle))
        score += 45;
    if (company.length >= 3 && sender.includes(company))
        score += 55;
...
function overlapScore(source, target, maximum) {
    if (!source.length)
        return 0;
