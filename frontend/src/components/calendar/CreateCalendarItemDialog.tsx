...
export function CreateCalendarItemDialog({ date, onClose }: { date: Date | null; onClose: () => void }) {
  ...
  const applications = useApplications({ sortBy: 'updatedAt', sortOrder: 'desc', page: 1, limit: 100 })
  const applicationOptions = [{ value: '', label: 'No linked application' }, ...(applications.data?.data ?? []).map((application) => ({ value: application.id, label: `${application.company} — ${application.jobTitle}` }))]
