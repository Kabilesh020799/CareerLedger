...
- Create applications with validated fields and an optional PDF, DOC, or DOCX resume attachment up to 5 MB.
...
| `POST` | `/api/applications` | Create an application, optionally with a multipart `resume` attachment |
...
| `PATCH` | `/api/applications/:id` | Update an application, optionally replacing its multipart `resume` attachment |
